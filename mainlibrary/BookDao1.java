...
public static int save(String callno,String name,String author,String publisher,int quantity){
	...
	try{
		...
		PreparedStatement ps=con.prepareStatement("insert into library.books(callno,name,author,publisher,quantity) values(?,?,?,?,?)");
    ...
    public static boolean PublisherValidate( String Publisher)
{
    ...
    try(Connection con = DB1.getConnection()) {
        PreparedStatement ps = con.prepareStatement("select * from library.Publisher where PublisherName = ?"); 
    ...
    public static int AddPublisher( String Publisher)
    {
        int status= 0;
        try(Connection con = DB1.getConnection()) {
		PreparedStatement ps=con.prepareStatement("insert into library.Publisher(PublisherName) values(?)");
		ps.setString(1,Publisher);
		status=ps.executeUpdate();
                con.close();
	}catch(Exception e){System.out.println(e);}
    ...
    public static int SaveBook(String BookN, String AuthorN, String PublisherN, String ShelfN, String RowN, String GenreN) {
            int status= 0;
        try(Connection con = DB1.getConnection()) {
		PreparedStatement ps=con.prepareStatement("insert into library.Books(BookName,Author,Genre,Publisher,Shelf, Row) values(?,?,?,?,?,?)");
    ...
    public static int Delete(int BookID)
    {
          int status= 0;
        try(Connection con = DB1.getConnection()) {
		PreparedStatement ps=con.prepareStatement("DELETE FROM library.Books where BookID=?");
		ps.setInt(1,BookID);
		status=ps.executeUpdate();
                con.close();
	}catch(Exception e){System.out.println(e);}
